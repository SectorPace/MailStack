...
export const DnsSetupWizard: React.FC<Props> = ({
  ...
  const getStepContent = (stepNum: number) => {
    switch (stepNum) {
      ...
      case 5: {
        const publicKey = dkimPublicKey.trim().replace(/^v=DKIM1;\s*k=rsa;\s*p=/i, '').replace(/\s+/g, '');
        return {
          type: 'TXT',
          host: `${dkimSelector}._domainkey.${domainName}`,
          hostShort: `${dkimSelector}._domainkey`,
          value: publicKey ? `v=DKIM1; k=rsa; p=${publicKey}` : '',
          selector: dkimSelector,
          noteZh: publicKey ? '使用服务器生成的真实 DKIM 公钥。某些 DNS 服务商限制单条 TXT 记录长度时，请按服务商要求拆分字符串。' : '服务器尚未提供 DKIM 公钥，请先完成身份配置并重新加载快照；当前不会生成演示密钥。',
          noteEn: publicKey ? 'Use the real DKIM public key generated by the server. Split long TXT values only as required by your DNS provider.' : 'The server has not provided a DKIM public key yet. Complete identity setup and refresh; no demo key is generated.'
        };
      }
      case 6:
              ...
              <div className="space-y-1 sm:col-span-2">
                ...
                <div className={`flex items-center justify-between p-2 rounded-xl border break-all ${
                  ...
                  <button
                    ...
                    disabled={!((currentStepData as any).value)}
                    className="p-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 hover:text-cyan-300 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed shrink-0 ml-2"
                    title="复制记录值"
                  >
